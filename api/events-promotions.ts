...
export async function GetAllEvents(): Promise<CoffeeEvent[]> {
  try {
    ...
    res.data = (res.data as CoffeeEvent[]).map(
      (booking: CoffeeEvent) => ((booking.startDate = new Date(booking.startDate)), booking)
    );
    res.data = (res.data as CoffeeEvent[]).map(
      (booking: CoffeeEvent) => ((booking.endDate = new Date(booking.endDate)), booking)
    );
    return res.data;
  } catch (err: any) {

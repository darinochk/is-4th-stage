    ...
    >
      <form
        ...
        onSubmit={(e) => {
          ...
          setRequestSent(true);
          setResponse(null);
          setUser(userInternal).then((resp) => {
            if (!resp.isError) {
              dialogRef.current?.close();
            } else {
              setResponse(resp);
      ...
      >
        ...
        <label>
          ...
          <input
            type="tel"
            name="phone"
            value={userInternal?.phone || ""}
            onChange={handleUserChange}
            onInput={PhoneValidator}
          />
        </label>

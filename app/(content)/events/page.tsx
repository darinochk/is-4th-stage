...
import {
    ...
    CreatePromotion,
    DeleteEvent,
    GetAllEvents, GetAllPromotions, Promotion,
    UpdateEvent
...
export default function Page() {
    ...
    const handleBlur: FocusEventHandler<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement> = (e) => {
    ...
    return (
        <div>
            ...
            {inited && isAdmin && (
                <form className={styles.write_desk + ' ' + (writing ? '' : styles.collapsed)}
                      onSubmit={(e) => {
                          ...
                          createFunction(eventData, setResponse)
                              .then(post => {
                                  if (post) {
                    ...
                    <input type="text" className={styles.text} name="name" maxLength={256} required
                    ...
                    <label>Описание события:
                        <textarea className={styles.text} name="description" required
                    ...
                    <label>Дата начала:
                        <input type="date" className={styles.text} name="startDate" required
                               onFocus={handleFocus}
                               onBlur={handleBlur}
                        />
                    </label>
                    <label>Дата окончания:
                        <input type="date" className={styles.text} name="endDate" required
                               onFocus={handleFocus}
                               onBlur={handleBlur}
                        />
                    </label>
            ...
            <div className={styles.desks__container}>
                {events.map(e => (e.event = true, e)).concat(promotions).map((event, index) => (
                    <EventCard isEvent={event.event === true} key={index} event={event} onChange={newEvent => {
                        if (newEvent) {
...
function EventCard({event, onChange, isEvent}: { event: CoffeeEvent, onChange: (event: CoffeeEvent | null) => void, isEvent: boolean }) {
    ...
    return (
        <div className={styles.desk} style={{gridTemplateColumns: "max-content 1fr"}}>
            ...
            <p>Дата начала:</p><p>{new Date(event.startDate).toLocaleDateString()}</p>
            <p>Дата окончания:</p><p>{new Date(event.endDate).toLocaleDateString()}</p>
            {isAdmin && <><button onClick={() => dialogRef.current?.showModal()}>Изменить</button>
            <button onClick={() => {
                onChange(null);
            }}>Удалить
            </button></>}
            <EventChangePopup event={event} ref={dialogRef} setEvent={ev => {

        ...
        <main className={styles.main}>
            ...
            <form className={styles.write_desk + ' ' + (writing ? '' : styles.collapsed)}
                  onSubmit={(e) => {
                      ...
                      CreateReview(Object.fromEntries(formData), setResponse)
                          .then(post => {
                              if (post) {
                ...
                <input type="text" className={styles.text} name="reviewText" maxLength={256} required
                ...
                <div>Ваша оценка:
                    <div className={styles.rating__chooser}>
                        <input type="radio" id="rating1" name="rating" value="1" required onFocus={handleFocus}
                               onBlur={handleBlur}/>
                        <label htmlFor="rating1">1</label>

                        <input type="radio" id="rating2" name="rating" value="2" required onFocus={handleFocus}
                               onBlur={handleBlur}/>
                        <label htmlFor="rating2">2</label>

                        <input type="radio" id="rating3" name="rating" value="3" required onFocus={handleFocus}
                               onBlur={handleBlur}/>
                        <label htmlFor="rating3">3</label>

                        <input type="radio" id="rating4" name="rating" value="4" required onFocus={handleFocus}
                               onBlur={handleBlur}/>
                        <label htmlFor="rating4">4</label>

                        <input type="radio" id="rating5" name="rating" value="5" required onFocus={handleFocus}
                               onBlur={handleBlur}/>
...
function ReviewCard({review}: { review: Review }) {
    ...
    return (
        <div className={styles.review + ' ' + (editing ? styles.editing : '')}>
            {isAdmin &&
                <img src={editing ? save.src : edit.src}
                     className={styles.edit_review} alt="Редактировать"
                     onClick={() => {
                         setEditing(!editing);
                         if (editing) {
            ...
            <a className={styles.review_author}>
                <img
                    src={no_photo.src}
                    alt="" width={50} height={50} style={{objectFit: 'cover', borderRadius: '100%'}}/>
            ...
            <pre contentEditable={editing} onInput={(e) => {
                setEditedReview({...editedReview, reviewText: e.currentTarget.innerText});
            }} suppressContentEditableWarning
                 className={styles.review_text + " " + styles.opened}>{editedReview.reviewText}</pre>
